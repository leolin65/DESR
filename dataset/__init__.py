...
def get_dataset(
    ...
    if stage == "train":
        ...
        meta_path = os.path.join("./dataset/metadata", dataset_name, "test.jsonl")
        # Medical metadata in some V25b packages is provided as full-shot.jsonl only.
        # Fall back automatically instead of crashing with FileNotFoundError.
        if not os.path.isfile(meta_path):
            fallback_meta = os.path.join("./dataset/metadata", dataset_name, "full-shot.jsonl")
            if os.path.isfile(fallback_meta):
                meta_path = fallback_meta
            else:
                raise FileNotFoundError(
                    f"No test metadata found for {dataset_name}. Tried {meta_path} and {fallback_meta}"
                )
